...
def compute_initial_positions(n, leader_id, arc_span, d):
    """
    Posiciones iniciales en formación arco con theta_leader=0 (líder mirando al este).
    Líder en (0, 0). Follower i en: ángulo = π + delta_i, distancia d desde el líder.
    Los seguidores se distribuyen uniformemente en el arco centrado detrás del líder.
    """
    n_followers = n - 1
    ...
    for i in range(1, n_followers + 1):
        ...
        angle = math.pi + delta
        x = round(d * math.cos(angle), 4)
        y = round(d * math.sin(angle), 4)
        positions[i] = (x, y)
...
def launch_setup(context, *args, **kwargs):
    ...
    arc_span      = float(LaunchConfiguration('arc_span').perform(context))
    ...
    init_pos = compute_initial_positions(n, leader_id, arc_span, d)
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('arc_span',
            default_value='3.14159',
            description='Apertura total del arco en radianes (π = semicírculo)'),
        DeclareLaunchArgument('d',
            default_value='1.5',
            description='Radio del arco: distancia de cada seguidor al líder (m)'),
        ...
        DeclareLaunchArgument('save_dir',
            default_value='figuras/formacion2',
