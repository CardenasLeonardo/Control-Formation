...
class PVA:
    ...
    def build_constraints(self, ranges):
        """
        Convierte las distancias LIDAR activas en restricciones (A, B, C).

        Los rayos se derivan directamente de len(ranges),
        cubren [-π, π) uniformemente, sin importar cuántos sean.
        """
        n = len(ranges)
    ...
    def solve_qp(self, v_ref, w_ref, constraints):
        ...
        return result.x if result.success else np.array([v0, w0])

    # ------------------------------------------------------------------

    def apply(self, v_ref, w_ref, ranges):
        """
        Construye restricciones desde el LIDAR, resuelve el QP y devuelve
        (v*, w*, constraints) para diagnóstico.
        """
        constraints = self.build_constraints(ranges) if ranges else []
        u = self.solve_qp(v_ref, w_ref, constraints)
        return float(u[0]), float(u[1]), constraints
