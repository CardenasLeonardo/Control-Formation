...
from control_nodes.algorithms.pivot_giro import PivotGiroWaypointsV2
...
def _make_sdf_sphere(name, color, radius=0.18, transparency=0.65):
...
class PivotGiroWaypointsV2Node(Node):
    """
    Igual que PivotGiroWaypointsNode, pero usando PivotGiroWaypointsV2:
    para cada waypoint, el giro (control proporcional del error angular,
    pivote relativo) y el avance en línea recta (v_max constante) ocurren
    A LA PAR en vez de en dos etapas secuenciales — la formación nunca
    se queda quieta girando, avanza mientras se orienta.
    """
    ...
    def __init__(self):
        super().__init__('pivot_giro_waypoints_v2_node')
        ...
        if (self.n < 3 or self.n % 2 == 0 or omega <= 0.0 or v_max <= 0.0
                or len(pos_flat) != 2 * self.n or len(wp_flat) < 2
                or len(wp_flat) % 2 != 0):
            raise ValueError(
                "Faltan o son inválidos los parámetros requeridos "
                "(n_robots impar >= 3, omega > 0, v_max > 0, "
                "initial_positions de longitud 2*n_robots, waypoints pares)"
            )
        ...
        self.giro = PivotGiroWaypointsV2(
        ...
        self.get_logger().info(
            f'PivotGiroWaypointsV2 | n={self.n}  waypoints={len(self.waypoints)}  '
            f'omega={omega}rad/s  v_max={v_max}m/s  start_delay={self.start_delay}s'
        )
    ...
    def _spawn_all(self):
        ...
        for i in range(self.n):
            ...
            req.name                       = name
            req.xml                        = _make_sdf_sphere(name, color, self.RADIUS)
        ...
        for i, (wx, wy) in enumerate(self.waypoints):
            ...
            req = SpawnEntity.Request()
            req.name                       = name
            req.xml                        = _make_sdf_diamond(name, GOLD, self.WP_SIZE)
            req.initial_pose.position.x    = wx
            req.initial_pose.position.y    = wy
            req.initial_pose.position.z    = self.WP_Z
            req.initial_pose.orientation.w = 1.0
            req.reference_frame            = 'world'
            self.spawn_cli.call_async(req)

        self.get_logger().info(f'Esferas y waypoints spawneados. Esperando {self.start_delay}s...')
