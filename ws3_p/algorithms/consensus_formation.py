...
class ConsensusFormation:
    """
    Formación en V con consenso relativo inter-agente y rate-limiting de anclas.

    Ley:
        eᵢ = Σⱼ[(xⱼ−rⱼ) − (xᵢ−rᵢ)]  +  β·[(xL + rᵢ) − xᵢ]

    El primer término coordina agentes entre sí.
    El segundo ancla a cada agente respecto al líder.

    El ancla rᵢ se rate-limita a anchor_speed m/s para evitar que un giro
    brusco del líder genere un destino "detrás" del robot (que forzaría reversa).
    """
    ...
    def step(self, state, neighbors_dict, leader_id, follower_index,
             n_followers, angle_v, d, beta):
        ...
        return self.compute(state, neighbors, leader_state, my_offset,
                            neighbor_offsets, beta)

    def compute(self, state, neighbors, leader_state, offset,
                neighbor_offsets=None, beta=1.5):
        xi, yi, theta = state
        rix, riy = offset
